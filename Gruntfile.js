...
module.exports = function ( grunt ) {
     ...
     * listed in `package.json` when you do `npm install` in this directory.
     */
    ...
    var taskConfig = {
        /**
         * We read in our `package.json` file so we can access the package
         * name and version. It's already there, so we don't repeat ourselves
         * here.
         */
        ...
        /**
         * The `copy` task just copies files from A to B. We use it here to
        ...
        /**
         * `grunt concat` concatenates multiple source files into a single
         * file.
         */
        concat: {
            /**
            ...
            build_css: {
                ...
                dest: "<%= build_dir %>/assets/<%= pkg.name %>-" +
                "<%= pkg.version %>.css"
            },
            /**
        ...
        index: {
             ...
             * `index.html`. The `src` property contains the list of
             * included files.
             */
            build: {
                dir: "<%= build_dir %>",
            ...
            /**
             * When it is time to have a completely compiled application,
             * we can alter the above to include only a single JavaScript
             * and a single CSS file. Now we're back!
             */
            compile: {
                dir: "<%= compile_dir %>",
                src: [
    ...
    grunt.registerMultiTask( "index", "Process index.html template",
        function () {
            ...
            var jsFiles = filterForJS( this.filesSrc ).map(
                function ( file ) {
                    return file.replace( dirRE, "" );
                });
            var cssFiles = filterForCSS( this.filesSrc ).map(
                function ( file ) {
                    return file.replace( dirRE, "" );
                });
